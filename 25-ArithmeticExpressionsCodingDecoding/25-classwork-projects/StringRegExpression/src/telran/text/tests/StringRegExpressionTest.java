...
import static telran.text.Calculator.*;
class StringRegExpressionTest {
	...
	@Test
	void arithmeticExprTest() {
		assertTrue("   3 +4/7 -20".matches(arithmeticExpression()));
		assertTrue("3+4/7-20".matches(arithmeticExpression()));
		assertTrue("   3+4/7-20	".matches(arithmeticExpression()));
		assertTrue("   3+4/7-20 * 10	".matches(arithmeticExpression()));
		assertTrue("   3+4/7-20 * 10/0".matches(arithmeticExpression()));
		assertTrue("   10 ".matches(arithmeticExpression()));
		assertFalse("3+4 & 20".matches(arithmeticExpression()));
		assertFalse("3+4 + ".matches(arithmeticExpression()));
		assertFalse("3+4 + s".matches(arithmeticExpression()));
		assertFalse("3+4 + 2.5".matches(arithmeticExpression()));
	}
	@Test
	void splitTest() {
		String str = " 1-2 3^4* 55*";
		str = str.trim();
		String arExpected[] = {"1", "2", "3", "4", "55"};
		String arActual[] = str.split("\\D+");
		assertArrayEquals(arExpected, arActual);
		
		
	}
	@Test
	void calculateTest() {
		assertEquals(-19, calculate("   3 +4/7 -20"));
		assertEquals(20, calculate("   3 +4/7 *20"));
		try {
			calculate("3 +4/0 *20");
			fail("Arithmetic Exception is required");
